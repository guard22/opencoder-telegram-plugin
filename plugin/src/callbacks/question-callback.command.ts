  ...
  (deps: CommandDeps & { questionTracker: QuestionTracker }) => async (ctx: Context) => {
    ...
    if (data.startsWith("session:")) {
      ...
      await deps.queue.enqueue(() =>
        ctx.answerCallbackQuery({ text: `Active session set: ${label}` }),
      );
    ...
    if (!session) {
      await deps.queue.enqueue(() =>
        ctx.answerCallbackQuery({ text: "Question session expired or invalid." }),
      );
      return;
    }
    ...
    if (action === "done") {
      if (currentAnswers.length === 0) {
        await deps.queue.enqueue(() =>
          ctx.answerCallbackQuery({ text: "Please select at least one option." }),
        );
        return;
      }
      ...
      if (!question.multiple) {
        ...
        await deps.queue.enqueue(() => ctx.answerCallbackQuery());
        await proceedToNext(ctx, deps, questionId, questionIndex);
      } else {
        ...
        try {
          await deps.queue.enqueue(() => ctx.editMessageReplyMarkup({ reply_markup: keyboard }));
        } catch (error) {
